...
#import <App360Payment/App360Payment.h>
...
- (void)requestForEbanking {
    ...
    [App360Payment makeBankingTransactionWithAmount:50000
                                            payload:payload
...
#warning Please change transaction id value below if you don't want to see 404 error
    
    NSString *transactionId = @"transaction-id";
    
    [App360Payment checkStatusOfTransaction:transactionId block:^(MOGPaymentResponseObject *responseObject, NSError *error) {
        if (!error) {
            NSLog(@"");
        } else {
            //Error
